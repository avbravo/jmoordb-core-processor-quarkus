 ...
 * @author avbravo
 */
public class RegexAnalizerUtil {
...
// </editor-fold>
    ...
    // <editor-fold defaultstate="collapsed" desc="Boolean rulesValid(Regex regex, List<String> tokenList, String nameOfMethod, Boolean havePaginationParameters, Boolean haveString)">
    /**
     ...
     * @return
     */
    public static Boolean rulesValid(Regex regex, List<String> tokenList, String nameOfMethod, Boolean havePaginationParameters, Boolean haveStringParameters) {
        Boolean isValid = Boolean.FALSE;
        try {
// Valida posicion por posicio los elementos
// text .like. @text
// text .like. @text .limit. pagination .skip. @pagination
//| size | iden       | LOG                       | @PARA        | BOOL             | ()|
//| ---  | ------     | ---                       |---           |---               |-- |
//|  3   | 0          | 1=.like.                  | 2            |--                | 1 |
...
// </editor-fold>
    ...
    // <editor-fold defaultstate="collapsed" desc="String  isValidWhereCharacters(Where where, List<? extends VariableElement> parameters, String nameOfMethod)">
    public static String isValidWhereCharacters(Regex  regex , List<? extends VariableElement> parameters, String nameOfMethod) {
        String message = "";
        try {
             if (regex.where().trim().indexOf(".limit.") != -1) {
                message = "Methods " + nameOfMethod + "() Not allowed .limit.";

            }
            if (regex.where().trim().indexOf(".skip.") != -1) {
                message = "Methods " + nameOfMethod + "() Not allowed .skip.";

            }
            if (regex.where().trim().indexOf(".order.") != -1) {
                message = "Methods " + nameOfMethod + "() Not allowed .order.";

            }
            if (regex.where().trim().indexOf(".by.") != -1) {
                message = "Methods " + nameOfMethod + "() Not allowed .by.";

            }
            if (regex .where().trim().indexOf("(") != -1) {
                message = "Methods " + nameOfMethod + "() Not allowed ) for use in @Regex. If you want to combine operations with () use @Regex instead.";
